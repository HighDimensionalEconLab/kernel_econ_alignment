...
def neoclassical_growth_neural(
    ...
    def G(model, t):
        c = model(t)[:, [0]]
        k = model(t)[:, 1:]
        dcdt = c * (f_prime(k) - (delta + rho_hat))
        dkdt = f(k) - delta * k - c
    ...
    for epoch in range(num_epochs):
        for i, time in enumerate(train):
            ...
            loss_ode = 0.4*res_c_dot.pow(2).mean() + 0.4*res_k_dot.pow(2).mean()
            loss_init = res_init.pow(2).mean()
            loss = loss_ode + 0.2 * loss_init
    ...
    c_test = np.array(q_hat(test_data)[:, [0]].detach())
