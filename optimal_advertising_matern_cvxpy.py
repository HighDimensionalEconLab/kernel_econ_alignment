...
# Pyomo-free DNLP port of optimal_advertising_matern. The advertising-capital
# FOC collocation is nonconvex (the bilinear (1-x)*u and mu*u terms, and the
# u**((1-kappa)/kappa) marginal-cost relation), so it is solved through cvxpy's
# DNLP interface (prob.solve(nlp=True, ...)).  solver_type defaults to UNO (unopy,
# in-process, filtersqp preset); cyipopt's interior point diverges on this model's
# free-sign control u, so IPOPT is not offered here.
NLP_SOLVERS = {
    "UNO": (cp.UNO, dict(preset="filtersqp")),
}


def optimal_advertising_matern_cvxpy(
    rho_hat: float = 0.11,
    c: float = 0.5,
    beta: float = 0.05,
    kappa: float = 0.5,
    ...
    # Decision variables, initialized at the pyomo starting point (zero kernel
    # coefficients, mu_0 = u_0 = 0) so the local NLP solve begins identically.
    ...
    # Affine kernel expansions (the pyomo mu/x/u/dmu_dt/dx_dt helpers inline).
    # x is the market-share state, mu the costate, u the advertising control.
    ...
    prob = cp.Problem(
        cp.Minimize(
            cp.quad_form(alpha_mu, cp.psd_wrap(K)) + cp.quad_form(alpha_x, cp.psd_wrap(K))
        ),
        [
...
if __name__ == "__main__":
    jsonargparse.CLI(optimal_advertising_matern_cvxpy)
