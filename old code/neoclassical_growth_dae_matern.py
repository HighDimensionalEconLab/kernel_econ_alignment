...
def neoclassical_growth_dae_matern(
    a: float = 1 / 3,
    delta: float = 0.1,
    r: float = 0.11,
    k_0: float = 1.0,
    ...
    m.alpha_r = pyo.Var(m.I, within=pyo.Reals, initialize=0.0)
    m.r_hat_0 = pyo.Var(
        within=pyo.NonNegativeReals, initialize=a * k_0 ** (a - 1) - delta
    )
    ...
    def r_hat(m, i):
        return m.r_hat_0 + sum(K_tilde[i, j] * m.alpha_r[j] for j in m.I)
    ...
    @m.Constraint(m.I)  # for each index in m.I
    def euler(m, i):
        return dc_dt(m, i) == c(m, i) * (r_hat(m, i) - r)

    @m.Constraint(m.I)  # for each index in m.I
    def interest_rate(m, i):
        return r_hat(m, i) == a * k(m, i) ** (a - 1) - delta

    @m.Objective(sense=pyo.minimize)
    def min_norm(m):  # alpha @ K @ alpha not supported by pyomo
        return lambda_k * sum(K[i, j] * m.alpha_k[i] * m.alpha_k[j] for i in m.I for j in m.I) + lambda_c * sum(K[i, j] * m.alpha_c[i] * m.alpha_c[j] for i in m.I for j in m.I) + lambda_r_hat * sum(K[i, j] * m.alpha_r[i] * m.alpha_r[j] for i in m.I for j in m.I)
    ...
    alpha_r = jnp.array([pyo.value(m.alpha_r[i]) for i in m.I])
    c_0 = pyo.value(m.c_0)
    r_hat_0 = pyo.value(m.r_hat_0)
    ...
    def kernel_solution(test_data):
        ...
        r_hat_test = r_hat_0 + K_tilde_test @ alpha_r
        return k_test, c_test, r_hat_test

    sol_benchmark = neoclassical_growth_benchmark(
        a, delta, r, 1.0, k_0, benchmark_grid
    )

    # Generate test_data and compare to the benchmark
    k_benchmark, c_benchmark = sol_benchmark(test_data)
    k_test, c_test, r_hat_test = kernel_solution(test_data)
...
if __name__ == "__main__":
    jsonargparse.CLI(neoclassical_growth_dae_matern)
